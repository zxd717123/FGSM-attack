...
def maximize_fy_fgsm_attack(C, model, loss, x, y, eps):
    # 使用.to(device) 方法将其中一个张量移动到指定的设备上，以与另一个张量保持一致。
    # 例如，如果希望两个张量都位于CUDA设备上,使用下面的代码：
    x = x.to(device)
    C = C.to(device)
    x.requires_grad = True

    outputs = model(x)
    #y_pred = outputs.clone().detach()  # 获取预测结果 y
    # 所以为了最大化f(y),我们需要计算它的相反数,使其最小化
    fy = torch.matmul(outputs, C)  # 计算 f(y) = C * y 的值 (1x69 and 69x1)

    model.zero_grad()
    # backward()会计算梯度的符号,梯度的方向是使损失减小。
    # backward()计算出来的梯度就会指向最大化f(y)的方向
    fy.backward()  # 计算 f(y) 关于 x 的梯度

    grad = x.grad.detach()
    attack_direction = torch.sign(grad)  # 确定攻击方向，即梯度的符号
    perturbation = eps * attack_direction  # 生成扰动项，即攻击方向乘以扰动幅度 eps
    adversarial_x = x + perturbation
    # adversarial_x = x + perturbation  # 对抗样本 = 原始样本 + 扰动项
    # if targeted == 1:
    #     adversarial_x = x - perturbation
    # else:
    #     adversarial_x = x + perturbation
...
def fgsm(targeted):
    # Predicted data
    # yvmtest（12，300）
    # C = torch.randint(1, 101, (69, 1)).float()
    C = torch.zeros((68, 1))
    C = torch.cat((C, torch.ones((1, 1))), dim=0).float()
    yPgtest_hat_adv = torch.zeros((Ntest, Nbus_Pg))
    # 使用enumerate可以同时得到每个batch的索引step和batch的数据(test_x, test_y)
    for step, (test_x, test_y) in enumerate(test_loader_Pg):
        loss = nn.MSELoss()
        eps = 0.008
        #test_x = maximize_fy_fgsm_attack(C, model_Pg, loss, test_x, test_y, eps, targeted).to(device)
        if targeted == 1:
            test_x = maximize_fy_fgsm_attack(C, model_Pg, loss, test_x, test_y, eps).to(device)
        else:
...
# yPgtest_hats_adv 是一个 12000x69 的二维数组
# 输出第 69 列的数据
yPgtests = yPgtests[0:500, 68]
print('yPgtests:', yPgtests)

targeted = 1
yPgtest_hats_adv = fgsm(targeted)
column_69_targetded = yPgtest_hats_adv[0:500, 68]
print(column_69_targetded)

targeted = 0
yPgtest_hats_adv = fgsm(targeted)
column_69 = yPgtest_hats_adv[0:500, 68]
print(column_69)

yPgtests = yPgtests.detach().numpy()
column_69_targetded = column_69_targetded.detach().numpy()
column_69 = column_69.detach().numpy()
index = np.argsort(yPgtests.flatten())

plt.figure(figsize=(8, 6))
plt.plot(np.arange(yPgtests.size), yPgtests.flatten()[index], 'r', label='Original Y')
plt.scatter(np.arange(yPgtests.size), column_69_targetded.flatten()[index], s=1, c='b', label='Prediction_targeted')
plt.scatter(np.arange(yPgtests.size), column_69.flatten()[index], s=1, c='g', label='Prediction_untargeted')
plt.legend(loc='upper left')
plt.grid()
plt.xlabel('Index')
plt.ylabel('Y')
